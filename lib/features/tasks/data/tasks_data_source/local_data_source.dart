...
abstract class LocalDataSource{
  ...
  Future<Unit>updateTask({required MyTask task , required String attribute , required String value});
...
class LocalDataSourceImpl implements LocalDataSource{
  ...
  Future<List<TaskModel>> getAllTasks()async {
    final List<Map<String,dynamic>> tasksOrFailure = await sqfliteHelper.readData("SELECT * FROM 'myTasks'");
  ...
  Future<Unit> deleteTask({required int id}) async{
    int response = await sqfliteHelper.deleteData("DELETE FROM 'myTasks' WHERE id=$id ");
    return checkResponseOfUpdatePostDelete(
      response: response,
      isDelete: true,
    );
  ...
  @override
  Future<Unit> updateTask({required MyTask task , required String attribute , required String value}) async{
    int response = await sqfliteHelper.updateData("UPDATE 'myTasks' SET '$attribute'='$value' WHERE id = ${task.id} ");
  ...
  Unit checkResponseOfUpdatePostDelete({required int response , bool isUpdate=false , bool isDelete=false  }) {
     if(response !=0){
      return unit;
    }else{
      if(isUpdate){
